...
image:
  src: "https://res.cloudinary.com/dx73a1lse/image/upload/v1691097664/blog/build-your-own-react-routerwebp_wzdy1w.webp"
...
```tsx title=App.tsx
const getCurrentPath = () => window.location.pathname;  // [!code highlight]

function App() {
  const [currentPathname, setCurrentPathname] = useState(getCurrentPath()); // [!code highlight]

  useEffect(() => { // [!code highlight]
    const onLocationChange = () => setCurrentPathname(getCurrentPath()); // [!code highlight]
    window.addEventListener(EVENTS.PUSHSTATE, onLocationChange); // [!code highlight]
    return () => { // [!code highlight]
      window.removeEventListener(EVENTS.PUSHSTATE, onLocationChange) // [!code highlight]
    }; // [!code highlight]
    }, []) // [!code highlight]
...
useEffect(() => {
  ...
  window.addEventListener(EVENTS.POPSTATE, onLocationChange); // [!code highlight]
  return () => {
    window.removeEventListener(EVENTS.PUSHSTATE, onLocationChange);
    window.removeEventListener(EVENTS.POPSTATE, onLocationChange); // [!code highlight]
  };
}, []);
```
...
const EVENTS = {
  PUSHSTATE: "pushstate",
  POPSTATE: "popstate", // [!code highlight]
};
```
...
export function getRelativeHref(src: string | PathObject) {
